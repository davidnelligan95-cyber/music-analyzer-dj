...
class MusicAnalyzerDJ:
    ...
    def show_context_menu(self, event):
        ...
        if item:
            ...
            context = tk.Menu(self.root, tearoff=0, bg='#2a2a2a', fg='white',
                            activebackground='#0066cc', activeforeground='white')
            
            context.add_command(label="▶ Load to Deck 1", command=lambda: self.load_to_specific_deck(1))
            context.add_command(label="▶ Load to Deck 2", command=lambda: self.load_to_specific_deck(2))
            context.add_separator()
            context.add_command(label="🔍 Analyze", command=self.analyze_selected)
            context.add_command(label="✏️ Edit Tags", command=self.edit_tags)
            context.add_command(label="🎵 Set Beatgrid", command=self.set_beatgrid)
            context.add_separator()
            context.add_command(label="➕ Add to Crate", command=self.add_to_crate)
            context.add_command(label="⭐ Set Rating", command=self.set_rating)
            context.add_separator()
            context.add_command(label="📊 Track Info", command=self.track_info)
            context.add_command(label="📁 Show in Folder", command=self.show_in_folder)
            context.add_separator()
            context.add_command(label="🗑️ Remove", command=self.remove_track)
            
            context.tk_popup(event.x_root, event.y_root)
    
    def select_folder(self):
        """Select folder and load tracks into the library treeview"""
        folder = filedialog.askdirectory(title="Select Music Folder")
        if not folder:
            return
        
        # Clear existing tracks in treeview
        for item in self.tree.get_children():
            self.tree.delete(item)
        
        self.tracks = [] # Clear internal track list
        
        # Load music files from the selected folder
        for root, dirs, files in os.walk(folder):
            for file in files:
                if file.lower().endswith(('.mp3', '.wav', '.flac', '.m4a', '.ogg', '.aiff')):
                    full_path = os.path.join(root, file)
                    self.tracks.append(full_path)
                    
                    # Insert into treeview (simulated data for now)
                    self.tree.insert("", tk.END, values=(
                        os.path.basename(file),
                        "Unknown Artist",
                        "Unknown Album",
                        str(random.randint(80, 150)), # Simulated BPM
                        "320.0kbps", # Simulated Bitrate
                        f"{random.randint(2, 6):02d}:{random.randint(0, 59):02d}.{random.randint(0, 99):02d}" # Simulated Length
                    ))
        
        messagebox.showinfo("Success", f"Loaded {len(self.tracks)} tracks from:\n{folder}")
    
    def load_files(self):
        """Load individual audio files into the library treeview"""
        ...
        if files:
            for file_path in files:
                ...
                self.tree.insert("", tk.END, values=(
                    ...
                    "320.0kbps",
                    f"{random.randint(2, 6):02d}:{random.randint(0, 59):02d}.{random.randint(0, 99):02d}"
                ))
            messagebox.showinfo("Loaded", f"Loaded {len(files)} files into the library.")
    
    def load_to_deck(self, event):
        """Double-click a track in the library to load it to a deck"""
        selection = self.tree.selection()
        if selection:
            item = self.tree.item(selection[0])
            track_filename = item['values'][0]
            
            # Find the full path of the selected track
            full_track_path = None
            for track_path in self.tracks:
                if os.path.basename(track_path) == track_filename:
                    full_track_path = track_path
                    break
            
            if full_track_path:
                response = messagebox.askquestion("Load Track",
                                                f"Load '{track_filename}'?\n\nYES = Deck 1\nNO = Deck 2")
                deck_to_load = self.deck1 if response == 'yes' else self.deck2
                deck_to_load.load_track(full_track_path) # Call the deck's load_track method
            else:
                messagebox.showwarning("Error", "Could not find full path for selected track.")
    
    def load_to_specific_deck(self, deck_number):
        """Load selected track to a specific deck from context menu"""
        selection = self.tree.selection()
        if selection:
            item = self.tree.item(selection[0])
            track_filename = item['values'][0]
            ...
            if full_track_path:
                deck_to_load = self.deck1 if deck_number == 1 else self.deck2
                deck_to_load.load_track(full_track_path)
            else:
                messagebox.showwarning("Error", "Could not find full path for selected track.")
    
    def analyze_files(self):
        """Analyze all files in the current library"""
        if not self.tracks:
            messagebox.showwarning("No Tracks", "Load tracks into the library first!")
            return
        messagebox.showinfo("Analyzing", f"Analyzing {len(self.tracks)} tracks...\n\n(Detecting BPM, Key, Energy. This would take time with real analysis.)")
    
    def analyze_selected(self):
        """Analyze selected track from context menu"""
        selection = self.tree.selection()
        if not selection:
            messagebox.showwarning("No Selection", "Select a track first.")
            return
        item = self.tree.item(selection[0])
        track_filename = item['values'][0]
        messagebox.showinfo("Analyzing", f"Analyzing: {track_filename}\n\n(Detecting BPM, Key, Energy.)")
    
    def edit_tags(self):
        """Edit ID3 tags of the selected track"""
        selection = self.tree.selection()
        if not selection:
            messagebox.showwarning("No Selection", "Select a track first.")
            return
        ...
        artist = simpledialog.askstring("Edit Tags", f"Track: {track_filename}\n\nArtist:", initialvalue=item['values'][1])
        if artist:
            title = simpledialog.askstring("Edit Tags", "Title:", initialvalue=item['values'][0])
            album = simpledialog.askstring("Edit Tags", "Album:", initialvalue=item['values'][2])
            
            if title and album:
                # Update treeview (simulated)
                self.tree.item(selection[0], values=(title, artist, album, item['values'][3], item['values'][4], item['values'][5]))
                messagebox.showinfo("Saved", f"Tags updated for:\n\nArtist: {artist}\nTitle: {title}\nAlbum: {album}")
            else:
                messagebox.showwarning("Incomplete", "Title and Album cannot be empty.")
    
    def set_beatgrid(self):
        """Open beatgrid editor for the selected track"""
        selection = self.tree.selection()
        if not selection:
            messagebox.showwarning("No Selection", "Select a track first.")
            return
        item = self.tree.item(selection[0])
        track_filename = item['values'][0]
        messagebox.showinfo("Beatgrid", f"Opening beatgrid editor for: {track_filename}\n\n(Feature in development)")
    ...
    def track_info(self):
        ...
        info += f"Song: {values[0]}\n"
        info += f"Artist: {values[1]}\n"
        info += f"Album: {values[2]}\n"
        info += f"BPM: {values[3]}\n"
        info += f"Bitrate: {values[4]}\n"
        info += f"Length: {values[5]}\n"
        
        messagebox.showinfo("Track Info", info)
    
    def add_to_crate(self):
        """Add selected track to a crate"""
        crate = simpledialog.askstring("Add to Crate", "Enter crate name:")
        if crate:
            messagebox.showinfo("Added", f"Track added to crate: {crate}")
    
    def set_rating(self):
        """Set rating for the selected track"""
        rating = simpledialog.askinteger("Rating", "Enter rating (1-5):", minvalue=1, maxvalue=5)
        if rating:
            messagebox.showinfo("Rating", f"Track rated: {'⭐' * rating}")
    
    def show_in_folder(self):
        """Open file explorer to the location of the selected track"""
        selection = self.tree.selection()
        if not selection:
            return
        ...
        if full_track_path:
            folder = os.path.dirname(full_track_path)
            os.startfile(folder) # Windows only
        else:
            messagebox.showwarning("Error", "Could not find file location for selected track.")
    
    def remove_track(self):
        """Remove selected track from the library list"""
        selection = self.tree.selection()
        if selection:
            if messagebox.askyesno("Remove", "Remove this track from the library list?"):
        ...
        file_path = filedialog.asksaveasfilename(
            title="Save Crate",
            defaultextension=".crate",
            filetypes=[("Serato Crate", "*.crate"), ("JSON File", "*.json"), ("All Files", "*.*")]
        )
        if file_path:
            # Here you would save self.tracks (full_path, metadata, etc.) to the file
            messagebox.showinfo("Saved", f"Crate saved to:\n{file_path}")
    
    def export(self):
        """Export the current playlist/library to a different format"""
        if not self.tracks:
            messagebox.showwarning("No Tracks", "No tracks in the library to export!")
            return
        
        file_path = filedialog.asksaveasfilename(
            title="Export Playlist",
            defaultextension=".m3u",
            filetypes=[("M3U Playlist", "*.m3u"), ("Rekordbox XML", "*.xml"), ("CSV File", "*.csv"), ("All Files", "*.*")]
        )
        if file_path:
            # Here you would export self.tracks to the chosen format
            messagebox.showinfo("Exported", f"Playlist exported to:\n{file_path}")
    
    def preferences(self):
        """Open the preferences/settings window"""
        messagebox.showinfo("Preferences", "Preferences Menu:\n\n• Audio Setup\n• MIDI Devices\n• Library Settings\n• Display Options")
    
    def change_layout(self, layout_type):
        """Change the layout of the decks (e.g., Horizontal, Vertical, 4 Deck)"""
        messagebox.showinfo("Layout Change", f"Switching to '{layout_type}' layout.\n\n(Layout changes are visual placeholders for now)")
    
    def toggle_fullscreen(self):
        """Toggle fullscreen mode for the application"""
        current_state = self.root.attributes('-fullscreen')
        self.root.attributes('-fullscreen', not current_state)
        messagebox.showinfo("Fullscreen", f"Fullscreen: {'ON' if not current_state else 'OFF'}")
    
    def new_crate(self):
        """Create a new empty crate"""
        crate_name = simpledialog.askstring("New Crate", "Enter name for the new Crate:")
        if crate_name:
            # Here you would add the new crate to your folder_tree
            messagebox.showinfo("Crate Created", f"New Crate: '{crate_name}' created.")
    
    def new_smart_crate(self):
        """Create a new smart crate based on rules"""
        messagebox.showinfo("Smart Crate", "Smart Crate Creator:\n\nDefine rules based on BPM, Genre, Rating, Year, etc.\n\n(Feature in development)")
    
    def import_playlist(self):
        """Import a playlist from a file"""
        file_path = filedialog.askopenfilename(
            title="Import Playlist",
            filetypes=[("M3U Playlist", "*.m3u"), ("Rekordbox XML", "*.xml"), ("Serato Crate", "*.crate"), ("All Files", "*.*")]
        )
        if file_path:
            # Here you would parse the playlist file and add tracks to the library
            messagebox.showinfo("Playlist Imported", f"Playlist imported from:\n{file_path}")
    
    def show_guide(self):
        """Display the user guide/manual"""
        messagebox.showinfo("User Guide", 
                            "Music Analyzer DJ - Serato Style\n\n" +
                            "Version 1.0.0 (Student Edition by Dave Nelligan)\n\n" +
                            "How to Use:\n" +
                            "1. Load tracks from 'Files' menu or 'SELECT FOLDER' button.\n" +
                            "2. Double-click tracks in the library to load them to decks.\n" +
                            "3. Use deck controls (Play, Cue, Hot Cues, Loops) for mixing.\n" +
                            "4. Right-click tracks for more options (Analyze, Edit Tags, etc.).\n" +
                            "5. Browse, Prepare, and History tabs are functional placeholders.")
...
Q/W        - Load to Deck 1/2
1-8        - Set/Trigger Hot Cues
Left/Right - Nudge/Seek

LIBRARY:
Enter      - Load to Deck 1
Shift+Enter- Load to Deck 2
...
Esc        - Exit Fullscreen"""
    ...
    def show_about(self):
        ...
        messagebox.showinfo("About Music Analyzer DJ",
                            "Music Analyzer DJ - Serato Style Edition\n\n" +
                            "Version 1.0.0 (Student Edition)\n\n" +
                            "Created by: Dave Nelligan\n\n" +
                            "Inspired by Serato DJ Pro interface.\n\n" +
                            "Features:\n" +
                            "• Dual decks with hot cues and BPM display\n" +
